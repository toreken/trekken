...
def fetch_n225():
    ...
    try:
        ...
        # 大引け後も yfinance の日足更新には時間差があるため、1分足で今日のOHLCを合成
        try:
            now_jst = pd.Timestamp.now(tz='Asia/Tokyo')
            today_date = pd.Timestamp(now_jst.date())  # tz なしの Timestamp（df.index に合わせる）
    ...
    try:
        ...
        if not (last_close != last_close):  # NaN check
            ...
            ax_main.annotate(
                ...
                bbox=dict(boxstyle='round,pad=0.3', facecolor=last_color,
                          edgecolor=last_color),
                verticalalignment='center', zorder=20,
                annotation_clip=False
            )
    except Exception as e:
        print(f"last close indicator error for {symbol}: {e}")
...
def make_chart_image_nq(df, symbol):
    ...
    try:
        ...
        print(f"last close indicator error for {symbol} (nq): {e}")
...
FUTURES_INDEX_TAB_SYMBOLS = ['ES1!', 'NQ1!', 'SPY', 'RSP', 'DIA', 'QQQ', 'QQQE', 'IWM', 'VTI', 'VT', 'N225']
